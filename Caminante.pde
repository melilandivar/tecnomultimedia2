class Caminante {
  ...
  void dibujar() {
    ...
    if ( contTramo < cadena.lista.size() ) {
      ...
      if (amp>=70) {  //ACÁ SE MODIFICAN LOS PUNTOS DE LA CURVA, DEPENDIENDO DEL AMP DE LA VOZ, UN MAP PARA QUE SEA MÁS GRADUAL

        float mod = map(amp, 70, 94, 0.0, 3.0);
        c.modificar(mod);
        if (cantidadPasos>70) {
          cantidadPasos--;
          println ("amp es:" +mod);
          println ("velocidad esta:" +cantidadPasos);
        } }else {
          cantidadPasos=100;
        }
